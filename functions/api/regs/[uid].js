...
import { buildParties, taipeiNow, isDungeon, addDays } from "../_party.js";
export async function onRequestPatch({ request, env, params }) {
  ...
  const rows = results.map(r => ({ ...r, bento: !!r.bento, role: r.role || "", removed: !!r.removed, squad: r.squad == null ? null : Number(r.squad) }));
  const parties = [...buildParties(rows, tw.date), ...buildParties(rows, addDays(tw.date, 1))];
  ...
  const isToday = party.members[0].date === tw.date;
  const closed = isToday && ((party.ok && party.departMin != null && tw.min >= party.departMin) || party.timeEnd < tw.min);
