...
// 回傳依等級高→低、角色 ID 排序：[{ charId, level, job, activity, skills, start, end }]
...
export async function onRequestGet({ request, env }) {
  ...
  const { results } = await env.DB
    .prepare(`SELECT charId, level, job, activity, skills, startHM AS start, endHM AS "end", ts
  ...
  const list = Object.values(best)
    ...
    .map(({ ts, ...r }) => ({ ...r, skills: String(r.skills || "").split(",").filter(Boolean) }));
  return json(list);
}
